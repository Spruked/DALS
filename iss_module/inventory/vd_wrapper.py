...
class VisiDataWrapper:
    ...
    def export_to_csv(self, data: List[Dict[str, Any]], filename: str = "vd_export.csv") -> Path | None:
        ...
        try:
            DataExporter.to_csv_sync(data, str(file_path))
            return file_path
        except Exception as e:
            self.logger.error(f"Failed to export to CSV for VisiData: {e}")
            return None
    ...
    def export_to_json(self, data: List[Dict[str, Any]], filename: str = "vd_export.json") -> Path | None:
        ...
        file_path = Path(self.export_folder) / filename
        try:
            DataExporter.to_json_sync(data, str(file_path))
            return file_path
        except Exception as e:
            self.logger.error(f"Failed to export to JSON for VisiData: {e}")
            return None
